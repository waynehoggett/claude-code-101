...
# The learner typed /diff inside Claude Code before committing. Claude Code appends
# every submitted input to <config dir>/history.jsonl; the line copies the 2.1.x shape.
New-Item -ItemType Directory -Path $ClaudeHome -Force | Out-Null
$historyLine = [ordered]@{
    display = '/diff'; pastedContents = @{}
    timestamp = [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
    project = "/workspace/ledger-$Language"; sessionId = [guid]::NewGuid().ToString()
} | ConvertTo-Json -Compress
[IO.File]::AppendAllText("$ClaudeHome/history.jsonl", $historyLine + "`n")
