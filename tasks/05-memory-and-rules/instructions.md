# Memory and where rules live

In the last two tasks you put one rule in a project CLAUDE.md. Rules can live at
several levels, from your whole machine down to a single folder, and Claude also keeps
notes of its own. Start Claude Code in your ledger project.
...
- **b. Add a rule for yourself.** Ask:

  ```
  Create my user-level CLAUDE.md with one rule: after changing any code, run the project's test suite and show me the result before reporting back.
  ```
...
- **User** (`~/.claude/CLAUDE.md`): your habits, every project. A verification habit
  belongs here.
- **Project** (`CLAUDE.md` in the repo): the team's rules, shared through git.
...
Any of them can pull in another file with `@path/to/file`.
:::

## 2 · Add a rule that only applies to the tests

Ask:

```
Add a CLAUDE.md inside the folder that holds the tests with one rule: every test sets up its own data and never depends on another test.
```

:::tip[Rules that load on demand]
A subfolder CLAUDE.md costs nothing until Claude reads or edits something in that
folder, so the root file stays short and the details live where they apply. For rules
that follow a file pattern rather than a folder, put a markdown file in
...
buries the rules that actually change Claude's behaviour.
:::
